import { arrayBufferToHex } from "./network";
import { reverse } from "./utils";

/**
 * Interface untuk header protokol
 */
export interface ProtocolHeader {
  ...
  version?: ArrayBuffer | null;
  isUDP?: boolean;
}

/**
 * Deteksi protokol dari buffer data
 */
export async function protocolSniffer(buffer: ArrayBuffer): Promise<string> {
  ...
  if (arrayBufferToHex(sselvDelimiter.buffer as ArrayBuffer).match(/^[0-9a-f]{8}[0-9a-f]{4}4[0-9a-f]{3}[89ab][0-9a-f]{3}[0-9a-f]{12}$/i)) {
 ...
 */
export function parseSsHeader(ssBuffer: ArrayBuffer): ProtocolHeader {
  ...
  switch (addressType) {
    case 1: // IPv4
      addressLength = 4;
      addressValue = Array.from(new Uint8Array(ssBuffer.slice(addressValueIndex, addressValueIndex + addressLength))).join(".");
      break;
    case 3: // Domain
 ...
 * Parse vless header
 */
export function parseSselvHeader(buffer: ArrayBuffer): ProtocolHeader {
  ...
  return {
    ...
    version: new Uint8Array([version[0], 0]).buffer as ArrayBuffer,
    isUDP: isUDP,
  };
}
 ...
 * Parse trojan header
 */
export function parseNajortHeader(buffer: ArrayBuffer): ProtocolHeader {
